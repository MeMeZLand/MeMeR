...
--Last edit 1/22/2026 17:08:53 UTC+8

--- Export Types ---

local UIVersion = "0.0.1"
...
local Compkiller = {
	...
	Windows = {},
	Scale = {
		Window = getgenv()["电脑UI大小"] or UDim2.new(0, 485,0, 565),
		Mobile = getgenv()["手机UI大小"] or UDim2.new(0, 450,0, 275),
		TabOpen = 185,
		TabClose = 85,
	},
...
if getgenv().devMode then
	...
	loadstring(readfile("MeMeRDev/Functions.lua"))()
else
	loadstring(game:HttpGet(getgenv().MeMeRStorage.."Modules/Functions.lua"))()
end

if getgenv().MeMeRConnections ~= nil then
    for i, v in pairs(getgenv().MeMeRConnections) do
        v:Disconnect()
    end
    table.clear(getgenv().MeMeRConnections)
else
    getgenv().MeMeRConnections = {}
end

if not isfolder("MeMeR") then
    makefolder("MeMeR")
end
if not isfolder("MeMeR/Music") then
    makefolder("MeMeR/Music")
end
if not isfolder("MeMeR/Configs") then
    makefolder("MeMeR/Configs")
end
if not isfolder("MeMeR/Assets") then
    makefolder("MeMeR/Assets")
end
if not isfolder("MeMeR/Temp") then
	makefolder("MeMeR/Temp")
end
...
Compkiller.LoadingAssets = {}

if getcustomasset then
    function Compkiller:LoadCustomAsset(url, assettype, usecurrent)
        if url and assettype and not table.find(Compkiller.LoadingAssets, url) then
            if assettype == "Music" then
                if usecurrent == true then
                    if not isfile("MeMeR/Music/"..string.split(url, "/")[#string.split(url, "/")]) then
                        if url:lower():sub(1, 4) == "http" then
                            table.insert(Compkiller.LoadingAssets, url)
                            local fileName = "MeMeR/Music/"..string.split(url, "/")[#string.split(url, "/")]
                            writefile(fileName, request({Url = url, Method = "GET"}).Body)
                            local result = getcustomasset(fileName, true)
                            table.remove(Compkiller.LoadingAssets, table.find(Compkiller.LoadingAssets, url))
                            return result
                        end
                    elseif string.len(readfile("MeMeR/Music/"..string.split(url, "/")[#string.split(url, "/")])) < 10 then
                        if url:lower():sub(1, 4) == "http" then
                            table.insert(Compkiller.LoadingAssets, url)
                            local fileName = "MeMeR/Music/"..string.split(url, "/")[#string.split(url, "/")]
                            writefile(fileName, request({Url = url, Method = "GET"}).Body)
                        ...
                        return getcustomasset("MeMeR/Music/"..string.split(url, "/")[#string.split(url, "/")], true)
                    end
                else
                    if url:lower():sub(1, 4) == "http" then
                        local fileName = "MeMeR/Music/"..string.split(url, "/")[#string.split(url, "/")].."%temp%"..math.random(1, 99999)
                        writefile(fileName, request({Url = url, Method = "GET"}).Body)
                        local result = getcustomasset(fileName, true)
                        delfile(fileName)
                        return result
            ...
            elseif assettype == "Asset" then
                if usecurrent == true then
                    if not isfile("MeMeR/Assets/"..string.split(url, "/")[#string.split(url, "/")]) then
                        if url:lower():sub(1, 4) == "http" then
                            table.insert(Compkiller.LoadingAssets, url)
                            local fileName = "MeMeR/Assets/"..string.split(url, "/")[#string.split(url, "/")]
                            writefile(fileName, request({Url = url, Method = "GET"}).Body)
                            local result = getcustomasset(fileName, true)
                            table.remove(Compkiller.LoadingAssets, table.find(Compkiller.LoadingAssets, url))
                            return result
                        end
                    elseif string.len(readfile("MeMeR/Assets/"..string.split(url, "/")[#string.split(url, "/")])) < 10 then
                        if url:lower():sub(1, 4) == "http" then
                            table.insert(Compkiller.LoadingAssets, url)
                            local fileName = "MeMeR/Assets/"..string.split(url, "/")[#string.split(url, "/")]
                            writefile(fileName, request({Url = url, Method = "GET"}).Body)
                        ...
                        return getcustomasset("MeMeR/Assets/"..string.split(url, "/")[#string.split(url, "/")], true)
                    end
                else
                    if url:lower():sub(1, 4) == "http" then
                        local fileName = "MeMeR/Assets/"..string.split(url, "/")[#string.split(url, "/")].."%temp%"..math.random(1, 99999)
                        writefile(fileName, request({Url = url, Method = "GET"}).Body)
                        local result = getcustomasset(fileName, true)
                        delfile(fileName)
                        return result
...
task.spawn(function()
    if Compkiller.LoadCustomAsset then
        local MusicEndedListeners = {}
        local AudioConnection;
        local CurrentMusic = nil
        local lastmusicid = nil
        local Music = Assets["Music"]
        local Links = Music["Links"]
        local ID = Music["ID"]

        function Compkiller:MusicEnded(callback)
            table.insert(MusicEndedListeners, callback)
        end

        function Compkiller:PlayRandomMusic()
            local newmusic = math.random(1, #Links)
			Compkiller:StopMusic()
            if lastmusicid ~= ID[newmusic] then
                lastmusicid = ID[newmusic]
				if not CurrentMusic or not CurrentMusic.Parent then
					local Audio = Instance.new("Sound")
					CurrentMusic = Audio
				end
                CurrentMusic.Parent = game
                CurrentMusic.SoundId = Compkiller:LoadCustomAsset(Links[newmusic], "Music", true)
                CurrentMusic:Play()
                AudioConnection = CurrentMusic.Ended:Once(function()
                    for i, func in pairs(MusicEndedListeners) do
                        func()
                    end
                end)
            else
                Compkiller:PlayRandomMusic()
            end
        end

        function Compkiller:StopMusic()
        ...
        function Compkiller:PlayMusic(musicid, customasset)
            Compkiller:StopMusic()
            if not CurrentMusic then
                if table.find(ID, musicid) and not table.find(Compkiller.LoadingAssets, table.find(Links, table.find(ID, musicid))) then
                    lastmusicid = musicid
					if not CurrentMusic or not CurrentMusic.Parent then
						local Audio = Instance.new("Sound")
						CurrentMusic = Audio
                    ...
                    CurrentMusic.Parent = game
                    CurrentMusic.SoundId = Compkiller:LoadCustomAsset(Links[table.find(ID, musicid)], "Music", true)
                    CurrentMusic:Play()
                    AudioConnection = CurrentMusic.Ended:Once(function()
                        for i, func in pairs(MusicEndedListeners) do
                            func()
        ...
        function Compkiller:DownloadAllMusics()
            for i, v in pairs(Links) do
                local Downloaded = Compkiller:LoadCustomAsset(v, "Music", true)
                repeat task.wait()
                until Downloaded
            end
        end

		function Compkiller:ChangePlaySpeed(speed)
			if CurrentMusic then
				CurrentMusic.PlaybackSpeed = speed
			end
		end

		function Compkiller:ChangePlayVolume(volume)
			if CurrentMusic then
				CurrentMusic.Volume = volume
			end
		end

        function Compkiller:GetPlaylist()
            return ID
        end
    end
end)
...
function Compkiller:_GetWindowFromElement(Element)
	for i,v in next , Compkiller.Windows do
...
function Compkiller:_AddColorPickerPanel(Button, Callback)
...
function Compkiller:_AddPropertyEvent(Target , Callback)
...
function Compkiller:_LoadDropdown(BaseParent, Callback)
...
function Compkiller:_LoadElement(Parent , EnabledLine , Signal)
	...
	function Args:AddButton(Config)
		...
		function Args:UpdateFunction(func)
	...
	function Args:AddParagraph(Config) -- request by Neptune
		Config = Compkiller.__CONFIG(Config, {
			Title = "Paragraph",
			Content = "",
		});
...
function Compkiller:ChangeHighlightColor(NewColor)
...
function Compkiller:ChangeRiskyColor(NewColor)
	Compkiller.Colors.Risky = NewColor;
...
function Compkiller:ChangeBlockColor(NewColor)
	Compkiller.Colors.BlockColor = NewColor;
...
function Compkiller:ChangeBGDBColor(NewColor)
	Compkiller.Colors.BGDBColor = NewColor;

	for i,v in next , Compkiller.Elements.BGDBColor do
		if v.Element and v.Property then
			v.Element[v.Property] = Compkiller.Colors.NewColor;
		end;
	end;
end;

function Compkiller:ChangeStrokeColor(NewColor)
	Compkiller.Colors.StrokeColor = NewColor;
...
function Compkiller:ChangeSwitchColor(NewColor)
	Compkiller.Colors.SwitchColor = NewColor;
...
function Compkiller:ChangeBlockBackground(NewColor)
	Compkiller.Colors.BlockBackground = NewColor;
...
function Compkiller:ChangeDropColor(NewColor)
	Compkiller.Colors.DropColor = NewColor;
...
function Compkiller:ChangeLineColor(NewColor)
	Compkiller.Colors.LineColor = NewColor;
...
function Compkiller.new(Config)
	...
	function WindowArgs:DrawContainerTab(TabConfig)
		...
		function TabArgs:DrawTab(TabConfig) -- Internal Tab
			TabConfig = Compkiller.__CONFIG(TabConfig,{
				Name = "Tab",
				Type = "Double",
	...
	function WindowArgs:DrawConfig(Configuration, Internal)
		...
		function TabArgs:_DrawConfig()
			...
			AuthorText.Text = "创建者: <font color=\"rgb(17, 238, 253)\">NoFi</font>"
			...
			function ConfigButton:SetInfo(Author , ConfigName)
				...
				AuthorText.Text = string.format("创建者: <font color=\"rgb(%s, %s, %s)\">%s</font>" ,R,G,B , tostring(Author));
	...
	function WindowArgs:SetMenuKey(new)
		Config.Keybind = new;
	end;

	function WindowArgs:GetMenuKey()
...
function Compkiller:GetConfig(Type)
	local ConfigFlags = {};
...
function Compkiller:Loader(IconId,SoundId,Duration,Volume)
	local CompKiller = Instance.new("ScreenGui")
